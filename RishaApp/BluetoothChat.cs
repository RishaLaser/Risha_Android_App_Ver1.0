...
namespace RishaApp
{
	...
	/// </summary>
	[Activity (Label = "Bluetooth Terminal", MainLauncher = false,
	           ConfigurationChanges=Android.Content.PM.ConfigChanges.KeyboardHidden | Android.Content.PM.ConfigChanges.Orientation)]
	//[Activity (Label = "Bluetooth Terminal" ,MainLauncher = false, Icon="@drawable/icon" , Theme="@android:style/Theme.Holo.Light.NoActionBar")]
	public class BluetoothChat : Activity
	{
		...
		{
			string rastercode = Picture._rastercode;
			SendMessage (new Java.Lang.String("raster"));
			Picture.lines = rastercode.Split(new char[] { ',' }, StringSplitOptions.RemoveEmptyEntries);
			Picture.totalsteps = Picture.lines.Length;
		...
		private void SendMessage2 (Java.Lang.String message)
		{
			...
			//Java.Lang.String k = new Java.Lang.String(message.Concat ("\\n").ToCharArray()); //(Java.Lang.String)bld.ToString ();
			//message = k;
