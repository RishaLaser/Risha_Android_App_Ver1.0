		...
		private class ConnectedThread : Thread
		{
			...
			public void SendMessage(Java.Lang.String message)
			{
				...
				for (int i = 0; i < message.Length (); i++) {
					b[0] = message.CharAt(i);
					SendMessage2 (new Java.Lang.String (b));
					//Thread.Sleep (1);
				}
			...
			private void SendMessage2 (Java.Lang.String message)
			{
			...
			private void SendMessage3 (byte [] bytesMsg)
			{
				if (bytesMsg.Length > 0) {
					// Get the message bytes and tell the BluetoothChatService to write
					Write (bytesMsg);
				}
			}
			public static string data = "";
			public bool rastering = false;
			public bool vectoring = false;
			//string[] lines;
			//int totalsteps = 0;
			int counter = 0;
			int linecount = 0;
			int RasterSleepTimeInMs = 1; //10 Default
			private bool SendRaster(string data)
			{   
				try
				{
					//Encoding enc = null;
					if (data.Contains("Establishing contact.."))
					{

						//strLog.Append(string.Format("Recieved Establishing contact..  \n"));
						//System.Threading.Thread.Sleep(1000);
						SendMessage(new Java.Lang.String("A"));
						//strLog.Append(string.Format("Sent {0}\n", "A\n"));
						//lblstatus.Text = "Established contact..";
						//lblmc.Text = "Engrave";
						linecount = Picture.lines.Length;
					}
					if (data == "A")
					{
					...
					{
						counter++;
						Thread.Sleep(RasterSleepTimeInMs);
						int len = Picture.lines[counter - 1].Length + 10;
						SendMessage4(len.ToString());
					...
					{
						Thread.Sleep(RasterSleepTimeInMs);
						SendMessage4(Picture.lines[counter - 1]);
				...
				{
					_service._handler.ObtainMessage (BluetoothChat.MESSAGE_READ, 0, -1, ex.Message)
						.SendToTarget ();
				}
			...
			public override void Run ()
			{
				...
				while (true) {
					try {
						...
						bytes = mmInStream.Read (buffer, offset, buffer.Length-offset-1);
	
						offset += bytes;

						if (buffer[offset-1] == '\r' || buffer[offset-1] == '\n')
						{
							Encoding enc = Encoding.Default;
							//data = enc.GetString(buffer);
							data = System.Text.Encoding.UTF8.GetString(buffer);//buffer.ToString();//.Substring(0,buffer.ToString().IndexOf("\n") - 2);
							data = data.Substring(0,data.LastIndexOf('\n'));
							if (data.Contains("Raster"))
							{
								rastering = true;
								//totalsteps = Picture.totalsteps;
								//lines = Picture.lines;
							}
							if (rastering)
							{
							...
							_service._handler.ObtainMessage (BluetoothChat.MESSAGE_READ, offset, -1, buffer)
								.SendToTarget ();
